...
public class Timer : MonoBehaviour
{
    ...
    private void SetDialsAndSlidersValuesMidway()
    {
        if (standardSlider)
        {
            standardSlider.maxValue = ReturnTotalSeconds();
    ...
    public void StartTimer(double timeToStartAt = 0)
    {
        if (!timerRunning && !timerPaused)
        {
            ...
            if (countMethod == CountMethod.CountDown)
            {
                timeRemaining = timeToStartAt;
            }
            else
            {
                timeRemaining = ReturnTotalSeconds() - timeToStartAt;
                SetDialsAndSlidersValuesMidway();
            }
        }
    }


    public void StartTimerCustom(double timeToSet)
    {
        if(!timerRunning && !timerPaused)
        {
            timerRunning = true;
